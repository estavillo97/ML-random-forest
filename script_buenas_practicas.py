...
def feature_engineering(data: pd.DataFrame) -> pd.DataFrame:
    '''Returns the dataset after the process of getting the features and
    dropping not important columns'''
    data = data.drop(
        columns=['patient_nbr', 'encounter_id', 'out', 'payer_code',
                 'payer_code_2', 'name', 'metformin-rosiglitazone',
                 'examide', 'citoglipton', 'US', 'metformin-pioglitazone',
                 'glimepiride-pioglitazone', 'medical_specialty',
                 'troglitazone', 'troglitazone_2', 'weight',
                 'metformin-pioglitazone_2', 'acetohexamide',
    ...
    # encode meds
    meds_to_encode = [
        'glyburide', 'miglitol', 'glipizide',  'metformin', 'acarbose',
    ...
    numeric_columns = [
        ...
        'number_emergency', 'num_procedures', 'number_outpatient',
    ...
    data = pd.get_dummies(
        data, columns=[
            'diag_1', 'diag_2', 'diag_3', 'race', 'gender',
...
def get_clean_diag(value) -> int:
    ...
    if value is pd.NA or value == '?':
        ...
        clean_diag = 'E' + str(trunc)
    elif value[0] == 'V':
        trunc = float(value[1:]) // 20
        clean_diag = 'V' + str(trunc)
    else:
        trunc = float(value) // 200
        clean_diag = str(trunc)
    return clean_diag
